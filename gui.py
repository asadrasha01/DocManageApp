...
class DocumentApp:
    def __init__(self, root):
        ...
        self.summary_slider = tk.Scale(ctrl_frame, from_=1, to=7, orient=tk.HORIZONTAL, label="Summary Length (sentences)")
        self.summary_slider.set(3)
        self.summary_slider.grid(row=0, column=4, padx=10)

        self.result_text = scrolledtext.ScrolledText(root, width=90, height=26, font=("Courier", 10))
    ...
    def upload_files(self):
        file_paths = filedialog.askopenfilenames(title="Select Document(s)",
                                                 filetypes=[("Supported files", "*.txt *.docx *.pdf *.png *.jpg *.jpeg")])
        if not file_paths:
            return
        ...
        for path in file_paths:
            try:
                ...
                summary = summarize_text(cleaned_text, sentence_count=self.summary_slider.get())
